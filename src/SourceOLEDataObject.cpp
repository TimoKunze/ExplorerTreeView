...
FindDataEntry:
	...
	if(!pDataEntry->pData) {
		...
		if(properties.pOwnerExTvw) {
			properties.pOwnerExTvw->Raise_OLESetData(static_cast<IOLEDataObject*>(this), formatID, pFormat->lindex, pFormat->dwAspect);
		}
	...
	if(properties.numberOfItemsToDisplay > 1) {
		...
		if(pEntry) {
			...
			*(pDataEntry->pFormat) = *pFormat;
			...
			if(properties.pOwnerExTvw) {
				properties.pOwnerExTvw->Raise_OLEReceivedNewData(static_cast<IOLEDataObject*>(this), formatID, pFormat->lindex, pFormat->dwAspect);
			}
...
void SourceOLEDataObject::SetOwner(ExplorerTreeView* pOwner)
{
	if(properties.pOwnerExTvw) {
	...
	switch(format.cfFormat) {
		...
		if(pEntry) {
			...
			*(pDataEntry->pFormat) = format;
			...
			if(properties.pOwnerExTvw) {
				properties.pOwnerExTvw->Raise_OLEReceivedNewData(static_cast<IOLEDataObject*>(this), formatID, format.lindex, format.dwAspect);
			}
