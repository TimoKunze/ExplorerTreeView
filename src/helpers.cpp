...
// TODO: Can't we speed this one up?
BOOL IsTreeViewItemTruncated(HWND hWndTvw, HTREEITEM hItem, LPTSTR pItemText/* = NULL*/)
{
	...
	*reinterpret_cast<HTREEITEM*>(&itemRectangle) = hItem;
	if(SendMessage(hWndTvw, TVM_GETITEMRECT, static_cast<WPARAM>(TRUE), reinterpret_cast<LPARAM>(&itemRectangle))) {
		...
		if(!pItemText) {
			pItemText = new TCHAR[MAX_ITEMTEXTLENGTH + 1];
		}
		...
		if(SendMessage(hWndTvw, TVM_GETITEM, 0, reinterpret_cast<LPARAM>(&item))) {
			...
			int textWidth = textSize.cx + (cxEdge + 2);
			if((itemRectangle.left + textWidth) > clientArea.Width()) {
				truncated = TRUE;
			}
			ReleaseDC(hWndTvw, dc);
		}
		if(freeBuffer) {
			delete[] pItemText;
		}
