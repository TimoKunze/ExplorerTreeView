...
#include "IItemComparator.h"
	...
	///
	//@{
	#define BEGIN_FILTERED_MSG_MAP(theClass, pListener) \
	public: \
		...
		{ \
			...
			(wasHandled); \
			if(pListener) { \
				hr = pListener->PreMessageFilter(hWnd, message, wParam, lParam, &lResult, &cookie); \
				if(SUCCEEDED(hr)) { \
	...
	#define FILTERED_ALT_MSG_MAP(messageMapID) \
						break; \
					case messageMapID:
	...
	#define FILTERED_COMMAND_HANDLER(id, code, func) \
						if(message == WM_COMMAND && id == LOWORD(wParam) && code == HIWORD(wParam)) { \
							wasHandled = TRUE; \
							lResult = func(HIWORD(wParam), LOWORD(wParam), reinterpret_cast<HWND>(lParam), wasHandled); \
							if(wasHandled) { \
								goto DoFinalProcessing; \
							} \
						}
	...
	#define FILTERED_NOTIFY_HANDLER(id, cd, func) \
						if(message == WM_NOTIFY && id == reinterpret_cast<LPNMHDR>(lParam)->idFrom && cd == reinterpret_cast<LPNMHDR>(lParam)->code) { \
							wasHandled = TRUE; \
							lResult = func(static_cast<int>(wParam), reinterpret_cast<LPNMHDR>(lParam), wasHandled); \
							if(wasHandled) { \
								goto DoFinalProcessing; \
							} \
						}
	...
	#define FILTERED_NOTIFY_ID_HANDLER(id, func) \
						if(message == WM_NOTIFY && id == reinterpret_cast<LPNMHDR>(lParam)->idFrom) { \
							wasHandled = TRUE; \
							lResult = func(static_cast<int>(wParam), reinterpret_cast<LPNMHDR>(lParam), wasHandled); \
							if(wasHandled) { \
								goto DoFinalProcessing; \
	...
	#define FILTERED_NOTIFY_CODE_HANDLER(cd, func) \
						if(message == WM_NOTIFY && cd == reinterpret_cast<LPNMHDR>(lParam)->code) { \
							wasHandled = TRUE; \
							lResult = func(static_cast<int>(wParam), reinterpret_cast<LPNMHDR>(lParam), wasHandled); \
							if(wasHandled) { \
								goto DoFinalProcessing; \
	...
	#define FILTERED_NOTIFY_RANGE_HANDLER(idFirst, idLast, func) \
						if(message == WM_NOTIFY && reinterpret_cast<LPNMHDR>(lParam)->idFrom >= idFirst && reinterpret_cast<LPNMHDR>(lParam)->idFrom <= idLast) { \
							wasHandled = TRUE; \
							lResult = func(static_cast<int>(wParam), reinterpret_cast<LPNMHDR>(lParam), wasHandled); \
							if(wasHandled) { \
								goto DoFinalProcessing; \
	...
	#define FILTERED_NOTIFY_RANGE_CODE_HANDLER(idFirst, idLast, cd, func) \
						if(message == WM_NOTIFY && cd == reinterpret_cast<LPNMHDR>(lParam)->code && reinterpret_cast<LPNMHDR>(lParam)->idFrom >= idFirst && ((LPNMHDR) lParam)->idFrom <= idLast) { \
							wasHandled = TRUE; \
							lResult = func(static_cast<int>(wParam), reinterpret_cast<LPNMHDR>(lParam), wasHandled); \
							if(wasHandled) { \
								goto DoFinalProcessing; \
	...
	#define FILTERED_REFLECTED_NOTIFY_HANDLER(id, cd, func) \
						if(message == OCM_NOTIFY && id == reinterpret_cast<LPNMHDR>(lParam)->idFrom && cd == reinterpret_cast<LPNMHDR>(lParam)->code) { \
							wasHandled = TRUE; \
							lResult = func(static_cast<int>(wParam), reinterpret_cast<LPNMHDR>(lParam), wasHandled); \
							if(wasHandled) { \
								goto DoFinalProcessing; \
	...
	#define FILTERED_REFLECTED_NOTIFY_ID_HANDLER(id, func) \
						if(message == OCM_NOTIFY && id == reinterpret_cast<LPNMHDR>(lParam)->idFrom) { \
							wasHandled = TRUE; \
							lResult = func(static_cast<int>(wParam), reinterpret_cast<LPNMHDR>(lParam), wasHandled); \
							if(wasHandled) { \
								goto DoFinalProcessing; \
	...
	#define FILTERED_REFLECTED_NOTIFY_CODE_HANDLER(cd, func) \
						if(message == OCM_NOTIFY && cd == reinterpret_cast<LPNMHDR>(lParam)->code) { \
							wasHandled = TRUE; \
							lResult = func(static_cast<int>(wParam), reinterpret_cast<LPNMHDR>(lParam), wasHandled); \
							if(wasHandled) { \
								goto DoFinalProcessing; \
	...
	#define FILTERED_REFLECTED_NOTIFY_RANGE_HANDLER(idFirst, idLast, func) \
						if(message == OCM_NOTIFY && reinterpret_cast<LPNMHDR>(lParam)->idFrom >= idFirst && reinterpret_cast<LPNMHDR>(lParam)->idFrom <= idLast) { \
							wasHandled = TRUE; \
							lResult = func(static_cast<int>(wParam), reinterpret_cast<LPNMHDR>(lParam), wasHandled); \
							if(wasHandled) { \
								goto DoFinalProcessing; \
	...
	#define FILTERED_REFLECTED_NOTIFY_RANGE_CODE_HANDLER(idFirst, idLast, cd, func) \
						if(message == OCM_NOTIFY && cd == reinterpret_cast<LPNMHDR>(lParam)->code && reinterpret_cast<LPNMHDR>(lParam)->idFrom >= idFirst && reinterpret_cast<LPNMHDR>(lParam)->idFrom <= idLast) { \
							wasHandled = TRUE; \
							lResult = func(static_cast<int>(wParam), reinterpret_cast<LPNMHDR>(lParam), wasHandled); \
							if(wasHandled) { \
								goto DoFinalProcessing; \
	...
	#define END_FILTERED_MSG_MAP(pListener) \
						break; \
					default: \
	...
	DoFinalProcessing: \
				...
				BOOL eatenMessage = wasHandled; \
				if(!wasHandled) { \
					if(hWnd == containedEdit) { \
						lResult = containedEdit.DefWindowProc(message, wParam, lParam); \
						wasHandled = TRUE; \
					} else if(hWnd == *this) { \
				...
				if(callPostMessageFilter && pListener) { \
					pListener->PostMessageFilter(hWnd, message, wParam, lParam, &lResult, cookie, eatenMessage); \
				} \
				return wasHandled; \
			} \
			return TRUE; \
...
/// \brief <em>Used to mark internal caret changes</em>
///
/// \sa ExplorerTreeView::AddInternalCaretChange
#define TVC_INTERNAL 100
/// \brief <em>Used to retrieve or set an item's \c lParam data as it is stored by \c SysTreeView32</em>
///
/// We give each item an unique ID, which is stored in the \c lParam member of its \c TVITEMEX
/// structure. The user-defined \c ItemData value is stored in an internal hash map. To make the
/// whole thing 100% transparent, we intercept \c TVM_GETITEM and \c TVM_SETITEM messages and return
/// (modify) the value out of (in) the hash map if \c TVIF_PARAM is set. However, this prevents us
/// from retrieving (setting) an item's unique ID, so we introduce a custom \c TVIF_* flag. If this
/// one is set, the \c ExplorerTreeView::OnGetItem (\c ExplorerTreeView::OnSetItem) method
/// will remove it and forward the message right to \c SysTreeView32.
///
/// \sa TreeViewItem::get_ID, TreeViewItem::get_ItemData, ExplorerTreeView::OnGetItem,
///     ExplorerTreeView::OnSetItem
#define TVIF_NOINTERCEPTION 0x40000000
#ifdef INCLUDESHELLBROWSERINTERFACE
	/// \brief <em>Used to tell \c OnInsertItem that the item being inserted is a shell item</em>
	///
	/// \c ShellTreeView needs to know whether the tree view item being inserted is a shell item or a normal
	/// one in order to support the non-shell item image list. Therefore we use a custom \c TVIF_* flag to
	/// tell \c ShellTreeView the item's type.
	///
	/// \sa ExplorerTreeView::OnInsertItem
...
///
/// \remarks If we change this value, we also need to update the Doxygen comments in ExTvw*.idl!
///
/// \sa TreeViewItem::get_Text
#define MAX_ITEMTEXTLENGTH MAX_PATH
...
/// \brief <em>Retrieves whether a treeview item's handle is a predefined one</em>
///
/// \param[in] hItem The item handle to check.
///
/// \return \c TRUE if the handle is a predefined one; otherwise \c FALSE.
#define ISPREDEFINEDHTREEITEM(hItem) \
    ((hItem == TVI_ROOT) || (hItem == TVI_FIRST) || (hItem == TVI_LAST) || (hItem == TVI_SORT))
...
///
/// \sa BUTTONANDSHIFT2OLEKEYSTATE, WPARAM2BUTTONANDSHIFT
void OLEKEYSTATE2BUTTONANDSHIFT(int bitField, SHORT& mouseButtons, SHORT& shiftButtons);

/// \brief <em>Generates OLE compatible bit fields for the state of the mouse and shift buttons</em>
///
/// \c MouseButtons holds the state of the three (left, right, middle) mouse buttons, \c shiftButtons
/// holds the state of the three (Shift, Ctrl, Alt) shift buttons. This function merges those information
/// into a single bit field of \c MK_* flags.
///
/// \param[in] mouseButtons A bit field holding the state of the mouse buttons.
/// \param[in] shiftButtons A bit field holding the state of the shift buttons.
/// \param[out] bitField A bit field that will be used to hold all button states.
///
/// \remarks The two input bit fields must be structured in the style of Visual Basic 6. This means:
///          - Bit 1 holds the state of the Shift button/left mouse button
///          - Bit 2 holds the state of the Ctrl button/right mouse button
///          - Bit 3 holds the state of the Alt button/middle mouse button
///
...
///
/// This function compares two treeview items. Its the callback function that we defined when we sent
/// the \c TVM_SORTCHILDRENCB message to the control.
///
...
/// \param[in] lParamSort Additional data that was passed to the control along with the
///            \c TVM_SORTCHILDRENCB message. Usually this is a pointer to the \c IItemComparator
///            implementation of the object that initiated the sorting.
///
...
///
/// \sa ExplorerTreeView::SortSubItems, IItemComparator::CompareItems
int CALLBACK CompareItems(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort);
/// \brief <em>Retrieves whether the specified treeview item's text is truncated</em>
///
/// \param[in] hWndTvw The treeview window the method will work on.
/// \param[in] hItem The treeview item to check.
/// \param[in,out] pItemText If not \c NULL, this buffer will receive the item's text. The buffer must be
///                large enough to hold \c MAX_ITEMTEXTLENGTH characters and the terminating 0.
///
/// \return \c TRUE if the item is truncated; otherwise \c FALSE.
///
/// \sa TreeViewItem::IsTruncated
BOOL IsTreeViewItemTruncated(HWND hWndTvw, HTREEITEM hItem, LPTSTR pItemText = NULL);
...
///
/// \sa ExplorerTreeView::OnGetDragImage, ExplorerTreeView::OLEDrag,
